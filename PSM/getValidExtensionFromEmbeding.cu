...
cudaError_t getValidExtensionFromEmbeding(struct_Q *device_arr_Q,int indexOfQ,cHistory **dH,int n,unsigned int maxOfVer,int *d_O,int *d_LO,int *d_N,int *d_LN,int numberOfElementd_O,int numberOfElementd_N,int lastColumn){
	...
	if(indexOfQ==lastColumn){
		...
		getValidForwardExtensionFromTheLastQ(device_arr_Q,indexOfQ,dH,n,maxOfVer,d_O,d_LO,d_N,d_LN,numberOfElementd_O,numberOfElementd_N);
	}
	else
	{
